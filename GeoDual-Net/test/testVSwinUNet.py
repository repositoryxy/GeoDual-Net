...
# Model configuration
MODEL_NAME = 'SwinUNet'
MODEL_WEIGHT_PATH = r"/root/autodl-tmp/Geo-SegViT/ComparedModels_U/SwinUNet_Vai_networksD/SwinUNet_Vai_256_256/iter30k_epo150_bs8_lr0.001_s1234/best_model.pth"
...
def vis_swinunet_result(img, mask, pred, file_name, save_path, mean, std):
    ...
    axes[2].set_title('SwinUNet Prediction', fontsize=12)
    axes[2].axis('off')
...
def main():
    ...
    test_dataset = SwinUNetTestDataset(
        root_path=ROOT_PATH,
        img_size=IMG_SIZE,
        transform=transform
    )
